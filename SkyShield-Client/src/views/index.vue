  ...
  <div class="app-container home">
    <el-row :gutter="20">
      <el-col :sm="24" :lg="24">
        <blockquote class="text-warning" style="font-size: 14px">
          欢迎使用 SkyShield（天穹之盾）动态信任评分管理系统。
          <br />
          本系统用于统一承载信任评分、风险研判、策略执行与结果追踪等核心业务。
          <!-- <br />
          当前前端项目已完成品牌壳层统一，后续可继续叠加业务模块、接口联调与可视化能力。
          <h4 class="text-danger">
            建议在联调前确认后端接口、菜单配置与权限数据已同步到当前环境。
          </h4> -->
        </blockquote>
    ...
    <el-row :gutter="20">
      <el-col :sm="24" :lg="12" style="padding-left: 20px">
        <h2>SkyShield 管理平台</h2>
        <p>
          SkyShield 聚焦“动态信任评分”业务场景，前端负责提供统一的管理入口、配置能力与结果展示能力。当前工程保留了成熟后台框架的基础设施能力，同时完成了面向“天穹之盾”的品牌切换，便于后续继续扩展评分模型、数据看板、策略编排和审计追踪等功能。
        </p>
        <p>
          <b>当前版本:</b> <span>v{{ version }}</span>
        </p>
        <p>
          <el-tag type="danger">SkyShield</el-tag>
        </p>
        <p>
          <el-button
            type="primary"
            icon="Cloudy"
            plain
            @click="showProjectLinkInfo('repository')"
            >项目仓库</el-button
          >
          <el-button
            icon="HomeFilled"
            plain
            @click="showProjectLinkInfo('documentation')"
            >项目文档</el-button
          >
        </p>
      </el-col>
    ...
    <el-row :gutter="20">
      <el-col :xs="24" :sm="24" :md="12" :lg="8">
        <el-card class="update-log">
          ...
          <div class="body">
            <p>
              <i class="el-icon-s-promotion"></i> 系统名称：SkyShield（天穹之盾）
            </p>
            <p>
              <i class="el-icon-user-solid"></i> 产品定位：动态信任评分管理系统
            </p>
            <p>
              <i class="el-icon-chat-dot-round"></i> 当前状态：前端品牌壳层已统一为 SkyShield
            </p>
            <p>
              <i class="el-icon-money"></i> 联调说明：请使用当前环境配置的后端接口与权限数据
            </p>
          </div>
        </el-card>
      </el-col>
...
function showProjectLinkInfo(type) {
  const messageMap = {
    repository: "SkyShield repository link is not configured yet.",
    documentation: "SkyShield documentation link is not configured yet."
  }
  ElMessage.info(messageMap[type] || "SkyShield link is not configured yet.")
}
</script>
